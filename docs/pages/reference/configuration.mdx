...
| `SLACKBOTV2_SLACK_SEARCH_ENABLED` | `slackbotv2.slackSearchEnabled`. | Registers invocation contexts for [temporary Slack search answers](/operate/slack-search). Defaults to `false`; requires `search:read.public` on the bot and an API synthesis model. |
| `SLACK_SEARCH_EPOCH` | `apiRs.extraEnv`. | Requires matching `centaur.slack_search_epoch` principal enrollment, initializes fresh harness state, and rejects legacy execution recovery. Configure with the reviewed permission migration before enabling search. |
| `SLACK_SEARCH_AUTO_ENROLL` | `apiRs.extraEnv`; default off. | With `SLACK_SEARCH_EPOCH` set, creates never-seen Slack channels and users already enrolled, on the epoch's isolated role, instead of refusing them. Existing unenrolled principals are still refused. See [Slack search](/operate/slack-search). |
| `SLACK_SEARCH_PROVIDER` | `apiRs.extraEnv`; default `openai`. | Explicit synthesis provider for temporary Slack search answers: `openai` uses `OPENAI_API_KEY`/`OPENAI_BASE_URL`; `anthropic` uses `ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL` (default `https://api.anthropic.com`, without `/v1`). No provider fallback. |
| `SLACK_SEARCH_MODEL` | `apiRs.extraEnv`. | API synthesis model for temporary Slack search answers. Defaults to `gpt-5.4-mini` for OpenAI and `claude-haiku-4-5-20251001` for Anthropic. |
...
| `SESSION_SANDBOX_WARM_POOL_OBSERVABILITY_ENABLED`, `SESSION_SANDBOX_WARM_POOL_REPO_CACHE_ACCESS` | `apiRs.sandboxWarmPoolObservabilityEnabled`, `apiRs.sandboxWarmPoolRepoCacheAccess`. | Capability profile warm sandboxes boot with; defaults to observability on and repo cache `all`. A session claims a warm sandbox only when its principal resolves to exactly this profile. Other sessions cold-start and count as `capabilities_mismatch` in `centaur_sandbox_warm_pool_claims_total`. |
